...
I derived a deep reinforcement learning algorithm from mathematical first principles.
I derived the SACLite loss functions from the principle of maximum-entropy and justified the use of LayerNorm with a neural-tangent-kernel-inspired analysis.
Compared to baseline actor-critic algorithms, my algorithm did not diverge in high-dimensional continuous control.  
...
We showed that Q-learning with function approximation has two previously unnoticed heteroscedastic Gumbel noise sources.
An algorithm accounting for these noise sources attained almost 2 times the aggregate asymptotic performance of the popular SAC baseline.  
